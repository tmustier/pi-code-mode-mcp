...
import { buildCatalog, createCatalogSearch, describeTool, publicCatalog } from "./catalog.ts";
...
export class CodeExecutor {
  ...
  async execute(request: ExecuteRequest): Promise<CallToolResult> {
    ...
    const searchCatalog = createCatalogSearch(catalog);
    ...
    const context = vm.createContext({
      ...
      search: (query: string, options: { server?: string; limit?: number } = {}) =>
        Object.freeze(searchCatalog(query, options).map(result => Object.freeze(result))),
